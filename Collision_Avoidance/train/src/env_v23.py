...
class Test(core.Env):
    ...
    def get_force_sensor(self,side):
        if side == 'r':
            ...
            return self.force self.torque self.sensor
        elif side =='l':
    ...
    def move(self, goal):
        self.goal = goal
        res = self.set_goal_client(self.__name)
    ...
    def set_goal(self):
        self.goal_start = [0.0, 55.36, -6.36, 0.0, 78.91, 56.61, -80.19, -14.50]# [0.33, 0.0, -0.56, 0, 0, 0, 1] 
        rpy = [0.0, 0.0, 0.0]
        res = self.set_goal_client(self.goal_start, rpy, self.__name)
    ...
    def set_old(self):
        self.start = [0.0, 63.02, -8.58, 0.0, 88.0, 63.42, -85.22, -9.45]# [0.33, 0.0, -0.5, 0, 0, 0, 1] 
        rpy = [0.0, 0.0, 0.0]
        res = self.move_cmd_client(self.start, rpy, self.__name)
    ...
    def step(self, a):        
        ...
        if self.workers == 'arm':
            self.set_object(hole2, (0.25, 0.0, 0.8), (0.706825181105, 0.0, 0.0, 0.707388269167)) #name?
        fail = False
    ...
    def get_reward(self, s, terminal, singularity):
        reword = 0.
