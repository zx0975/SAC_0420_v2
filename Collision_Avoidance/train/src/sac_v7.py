...
class ValueNetwork(object):
    ...
    def step(self, obs):
        with tf.variable_scope(self.name):
            h1 = tf.layers.dense(obs, 128, tf.nn.leaky_relu)
            h2 = tf.layers.dense(h1, 128, tf.nn.leaky_relu)
...
class QValueNetwork(object):
    ...
    def step(self, obs, action, reuse):
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            h1 = tf.layers.dense(input, 128, tf.nn.leaky_relu)
            h2 = tf.layers.dense(h1, 128, tf.nn.leaky_relu)
...
class ActorNetwork(object):
    ...
    def step(self, obs, log_std_min=-20, log_std_max=2):
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            h1 = tf.layers.dense(obs, 128, tf.nn.leaky_relu)
            h2 = tf.layers.dense(h1, 128, tf.nn.leaky_relu)
            h3 = tf.layers.dense(h2, 128, tf.nn.leaky_relu)
